...
def fields_to_dwc(excel):
    ...
    # 处理未能找到对应关系的表头列名
    if hand_to_std != []:
        print("若干 Excel 列表名未能自动匹配标准字段名，请手动指明对应关系：\n\n")
        for n, v in enumerate(basic_columns):
            ...
            if (n+1)%2 > 0:
                ...
                print(strings.ljust(25), end="\n")
        print("\n请在相应列名后录入与上方标准字段名对应的序号，您也可以拆分 Excel 中的列，或者忽略该字段（请输入 0），或者新增 DWC 字段名：\
            \n\033[32m示例：\033[0m例如要对应 Excel 中“区县”列，应该录入序号“7”或者“county”；\
            \n\033[32m示例：\033[0m如果上述序号没有对应的 Darwin Core 列名，您可以直接输入符合 DWC 要求的字符，比如“eventDate”\
            \n      有关 DWC 囊括的字段及其意义请自行访问 \033[32m http://rs.tdwg.org/dwc/terms/\033[0m 查看；\
            \n\033[32m示例：\033[0m如果需要拆分 Excel 中的“省市”列，则需输入“staeteProvince,city”,其中的“,”为原表中省、市之间\的实际分隔符，如\
            \n      果您的数据是以“;”分割，请替换为“;”，而“stateProvince/city” 则是符合 Kingdonia 或 DWC 数据规范的列名。\n\n")
        to_json_clm = []
...
def merge_or_split_columns(table, txt, raw_to_std, basic_columns, filter=True):
    dwc = read_excel(os_path_dirname(sys_argv[0]) + "/./term_versions.xlsx")
    elements = re_compile("([a-z][a-zA-Z]*)([^a-zA-z]*)").findall(txt)
    ...
    if separators[-1] == "=":
        ...
        if filter == True:
            if False in [(t in dwc[dwc.status=="recommended"]["label"].values) for t in titles] or False not in [(t in basic_columns) for t in titles]:
                return "输入的列名不在 darwincore 标准之内，请重新输入：\n"
        ...
        raw_to_std.update(dict([(t, t) for t in titles]))
