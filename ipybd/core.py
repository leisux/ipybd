...
class FormatTable:
    ...
    def merge2pairs(self, columns, new_header, typ='dict'):
    ...
    def merge_columns(self, columns, separators, new_header):
        if separators == 'r':
            mergers = self.merge2pairs(columns, new_header, typ='rowList')
        elif separators == 'o':
            mergers = self.merge2pairs(columns, new_header, typ='jsonObject')
        elif separators == 'a':
            mergers = self.merge2pairs(columns, new_header, typ='jsonArray')
        elif separators == 'l':
            mergers = self.merge2pairs(columns, new_header, typ='list')
    ...
    def prt_items(self, col_num, seq, seq_num = None):
    ...
    def __unmapped_columns2std(self, unmapped_fields:list):
        ...
        self.prt_items(2, unmapped_fields, self.raw_columns)
    ...
    def build_mapping_template(self):
        ...
        while invalid_mapping:
            #print("\n下方式是可以映射的标准列名序列表：\n")
            #self.prt_items(2, self._stdfields)
