...
ipybd 是一款由 `Python` 语言开发的中文生物多样性数据清洗、统计与分析框架。当前 `ipybd` 的主要功能包括：

1. 通过物种名称，在线获取 [POWO](http://www.plantsoftheworldonline.org/), [IPNI](https://www.ipni.org/), [中国生物物种名录](http://www.sp2000.org.cn/)上最新的分类阶元、分类处理、物种图片、发表文献、相关异名等数据；
2. 对各类手工转录的经纬度、中文行政区划、人名、拉丁学名、日期与时间、各类点选值（如植物习性）、各类数值/数值区间（如海拔）等脏数据进行自动化的清洗、校正和转换；
3. 对数据表中指定的列进行各种类型的合并和拆分；
4. 将绝大多数不同结构不同数值格式的生物多样性数据表重塑为符合 Darwin Core 规范的数据表；
5. 定义了一套简洁的枚举语义，可供用户自行定制个性化的标准数据模版。只需若干行代码，即可生成高度自动化的 ETL 类。
6.  自定义的 ETL 类不仅可以实现不同结构的数据表到标准数据表之间的映射，同时还支持将单列、多列或整个表格数据列映射为各类 Python `dict`/`list` 对象或者 Json 对象和数组，以便于程序执行进一步的数据处理和分析 or 对接各类网络平台的 API。
7. `ipybd`基底数据结构基于 `Pandas.DataFrame` ，可以无缝调用 `Pandas` 生态的数据统计分析功能，同时也为未来`ipybd`自身的数据利用模块构建提供了强健的构造基础。

目前 `ipybd` 主要聚焦于构建众源数据集与生物多样性数据库之间的通道。在此之前，数据的提取、转换、装载（ETL）主要依赖于人力。数据链路中的采集者、接收者、利用者都需要为实现数据的标准化付出巨大的精力和时间（这一流程至少会占据 70% 的相关工作时间），而所执行的方案通常还无法被复用至不同的数据集。为此 `ipybd` 实现了一个**通用**的生物多样性数据提取、转换、装载框架，它可以显著提升数据平台（如 CVH/NSII）、数据接收机构（如标本馆）、数据利用者（如科研工作者）对不同来源、不同格式、不同品质、不同规范的数据集进行自动化的清洗转换与整合利用能力，从而大幅降低数据预处理的门槛和成本，加快数据利用的效率和品质。
...
目前 FormatTable 可接受一个 Excel、CSV、RDBMS 数据库对象进行实例化：基于 Excel 、CSV 数据表实例化，可以直接传递相应文件的路径给 `FormatTable`：

```python
collections = FormatTable(r"~/Documents/record2019-09-10.xlsx") 
```

`FormatTable` 默认采用 UTF-8 编码文件，如果传递 CSV 文件出现`UnicodeDecodeError`错误，可以尝试显示指定相应的编码方式，一般都可以得到解决。所有 Python 可选用的标准编码可[戳这里](https://docs.python.org/3/library/codecs.html#standard-encodings)。
...
基于本地或线上的关系型数据库创建 `FormatTable` 实例，需要先创建数据库连接，在 python 生态中有很多数据库连接库，比如 mysqlclient、pymysql、mysql-connector 等等，下方示例使用 sqlalchemy 建立 mysql 数据库连接，个人可以根据喜好自行选择相应的连接库创建连接器。
...
conn = create_engine('mysql+pymysql://root:passward@localhost:3306/ScientificName')

# 然后建立检索的 sql 语句，用于筛选出需要的数据
# 这里演示了从 ScientificName 数据库中调取 10 条 theplantlist 学名数据
sql = "select * from theplantlist limit 10;"

# 将 sql 语句和建立好的连接器传递给 ipybd.FormatTable
tpl = FormatTable(sql, conn)

# 执行完毕后，数据会以 DataFrame 的形式保存到实例的 df 对象
# 然后就可以在本地内存中操作这些数据
# 比如查看所调取数据中学名的 id 、family、genus 和拉丁名信息
...
`FormatTable` 类基于 `BioName` 实例封装了一些学名处理方法，以便用户能够更便捷的对数据表中的名称进行处理。比如对于上述 `collections` 实例，若相关数据表中的学名并非单列，而是按照 `"属名"`、`"种名"`、`"种下单元"`、`"命名人"`四列分列存储，使用 `BioName` 类需要先自行合并相应数据列才可以执行查询。而 `FormatTable` 实例则可以直接进行学名的查询和匹配：
...
如上所示，不同数据表的学名表示方式时常会有差异，使用 `FormatTable` 的实例方法可以大幅提高数据处理的便捷性和灵活性。目前 `FormatTable` 实例共支持以下几种学名处理方法：
...
如果并不希望程序直接返回结果，而是想直接将查询结果写入`collections`数据表，请求时可以将`concat`参数设置为`True`:
...
UTC 世界协调时在处理和分析跨时区生物多样性数据时，具有明显优势。同时 UTC 时间支持`JSON`数据集，利于数据交换。

#### 3.5 经纬度清洗和转换

经纬度数据是物种分布信息最为关键的信息，`FormatTable`为此提供了严格可靠的数据清洗方法，以保证在实现最大限度的数据自动清洗和转换的前提下，同时实现百分之百的数据纠错：
...
```
`FormatTable`实例可以调用`format_latlon`方法清洗经纬度数据列。调用时需要将经纬度涉及的一列或两列的列名传递给方法，`format_latlon`会自动纠正经度写在纬度列或者纬度写在经度列的值，同时 `format_latlon`对经纬度的格式也没有一致的要求，值的书写可以是十进制格式、度分格式、度分秒格式，或者以上几种的混合，同时对各种格式的分隔符也没有统一的要求。
